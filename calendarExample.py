...
class GCalendar:
  ...
  def _GetOwnCalendars(self):
    ...
    for a_calendar in feed.entry:
      l.append(a_calendar)
      print a_calendar.title.text
    return l

  def _PrintAllEventsOnCalendar(self, href=None):
  ...
  def Run(self, delete='false'):
    ...
    cal_list = self._GetOwnCalendars()
    for index in cal_list:
      self._PrintAllEventsOnCalendar(index.GetAlternateLink().href)
      break;
    """
    self._FullTextQuery()
...
def main():
  ...
  user ="haginara@gmail.com"
  pw = "cool1210"

  sample = GCalendar(user, pw)
  sample.Run()
